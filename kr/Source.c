...
int kol_och(float** res, int x);// поиск спортсменов, набравших более или менее введенного количества очков
void sort(int** points, int maxpoint[WIDTH], float sum, float** res, int numbigrok[HEIGHT]);//сортировка спортсменов по рейтингу (по убыванию или по возрастанию)
...
int main() {
	...
	while (c != 10) {
		...
		case 6:
			puts("\n");
			puts("1)Спортсмены, набравшие более введенного количества очков\n2)Спортсмены, набравшие менее введенного количества очков");
			scanf("%d", &x);
			kol_och(res, x);
			puts("\n");
			break;
